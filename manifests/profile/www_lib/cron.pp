...
class nebula::profile::www_lib::cron (
  ...
  cron {
    ...
    'staff.lib parse':
      hour    => 3,
      minute  => 30,
      command => "/usr/bin/perl /www/staff.lib/web/sites/staff.lib.umich.edu/local/dashboard/parse.pl > /www/staff.lib/web/sites/staff.lib.umich.edu/local/dashboard/logs/${facts['hostname']}-parse.log 2>&1",
    ;
