...
class nebula::profile::hathitrust::cron::mdp_misc (
  ...
  cron {
    ...
    'manage mbook sessions':
      ...
      command     => "${home}/scripts/managembookssessions.pl -m clean -a 120 2>&1 || /usr/bin/mail -s '${facts['hostname']} managembooksessions error' ${mail_recipient}";
    ...
    'harvest proxy downloads':
      ...
      hour        => '00',
      environment => $sdr_environment + ["MAILTO=''"],
      command     => "${sdr_root}/pt/scripts/harvest_proxy_downloads.pl 2>&1 || /usr/bin/mail -s '${facts['hostname']} harvest_proxy_downloads problems' ${mail_recipient}";
