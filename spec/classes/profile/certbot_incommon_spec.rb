  ...
  on_supported_os(supported_os: Nebula.supported_os).each do |os, os_facts|
    context "on #{os}" do
      ...
      context "with a single cert" do
        ...
        it do
          expect(subject).to contain_file("/tmp/all_cert_commands_incommon")
            .with_content(
              <<~CREDENTIALS
                certbot certonly --standalone --server "https://acme.sectigo.com/v2/InCommonECCOV" -m "our_real_email@email.gov" -d "example.invalid,*.example.invalid"
              CREDENTIALS
            )
        end
      ...
      context "with a multiple certs and services" do
        ...
        it do
          expect(subject).to contain_file("/tmp/all_cert_commands_incommon")
            .with_content(
              <<~CREDENTIALS
                certbot certonly --standalone --server "https://acme.sectigo.com/v2/InCommonECCOV" -m "nope@nope.zone" -d "abc.invalid,*.abc.invalid,abc.example,*.abc.example"
                certbot certonly --standalone --server "https://acme.sectigo.com/v2/InCommonECCOV" -m "nope@nope.zone" -d "abc.com,*.abc.com"
                certbot certonly --standalone --server "https://acme.sectigo.com/v2/InCommonECCOV" -m "nope@nope.zone" -d "zyx.invalid,*.zyx.invalid"
              CREDENTIALS
            )
        end
      ...
      context "with one simple cert, no SANs" do
        ...
        it do
          expect(subject).to contain_file("/tmp/all_cert_commands_incommon")
            .with_content(%r{certbot certonly --standalone --server "https://acme.sectigo.com/v2/InCommonECCOV" -m "nope@nope.zone" -d "abc.example"})
        end
      ...
      context "with one simple cert, two SANs" do
        ...
        it do
          expect(subject).to contain_file("/tmp/all_cert_commands_incommon")
            .with_content(
              <<~CREDENTIALS
                certbot certonly --standalone --server "https://acme.sectigo.com/v2/InCommonECCOV" -m "nope@nope.zone" -d "abc.example,san.local,xyz.local"
              CREDENTIALS
            )
        end
      end
      ...
      context "with two simple certs" do
        ...
        it do
          expect(subject).to contain_file("/tmp/all_cert_commands_incommon")
            .with_content(%r{certbot certonly --standalone --server "https://acme.sectigo.com/v2/InCommonECCOV" -m "nope@nope.zone" -d "xyz.example,alt.example,\*.alt.example"})
        end
