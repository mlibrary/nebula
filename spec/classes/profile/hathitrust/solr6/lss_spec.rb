  ...
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      it { is_expected.to contain_file('/usr/local/bin/index-release')
        ...
        .without_content(/if \[ \${SEGMENTS} -eq 0 \];/)
           .with_content(/if \[ \${SEGMENTS} -lt 1 \] \|\| \[ \${SEGMENTS} -gt 2 \];/)
      ...
      context 'on non-primary node' do
        ...
        it {
          is_expected.to contain_file('/usr/local/bin/index-release').without_content(%r|^touch /htapps/babel/flags/web/lss-release-\${TODAY}$|)
        }
      end
